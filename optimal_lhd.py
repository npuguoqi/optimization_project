...
def latin_hypercube_sampling(n,k,type=1):     #N represents the number of rows，k represents the number of columns
    result = np.empty([n, k])
    temp = np.empty([n])
    d = 1.0
    if type==1:
...
def generate_sysmetrix_latin_matrix(n,k):
    ...
    for j in range(k):
        ...
        for k in range(n):
            temp[k] = (k * d + (k + 1) * d) / 2
        #symmetric handling
        #for example: n=6,temp=[0.5,1.5,2.5,3.5,4.5,5.5] can satisfy
        #if n is even
        if n%2==0:
            ...
            for i in range(0,math.floor(n/2)):
                ...
                temp_matrix[n-1-i][j]=n-temp_matrix[i][j]
    return temp_matrix
#generate a good starting design
...
#calculate the distance between two points, p=1 is rectangle distance and p=2 is Euclidean distance
...
def cal_d_mean(n,k):
    return (n+1)*k/3
#calculate the lower bound of phi_q
def cal_phi_q_l(n,k,q):
    ...
    return phi_q_l
#calculate the upper bound of phi_q
def cal_phi_q_u(n,k,q):
    ...
    return phi_q_u
#calculate phi_q
def cal_phiq(sampling_matrix,q,p):
    '''
    phi_q=0.0
...
def simulated_annealing_multiobj(sampling_matrix,T0,T_MIN,I_MAX,FAC_T,q,p):
    w=0.5
    n=sampling_matrix.shape[0]
    k=sampling_matrix.shape[1]
    phi_q_l=cal_phi_q_l(n,k,q)
...
#generate data by LHD and bounds
def generate_data(sampling_matrix,bounds):
    n=sampling_matrix.shape[0]
    k=sampling_matrix.shape[1]
    temp_sampling_matrix=sampling_matrix/n
    lower_bounds=bounds[:,0]
    upper_bounds=bounds[:,1]
    data_result=np.multiply(temp_sampling_matrix,upper_bounds-lower_bounds)
    data_result=np.add(data_result,lower_bounds)
    return data_result

if __name__=='__main__':
    #set number of levels and number of factors
    n=13
    k=2
    #for test
    if k==2:
    ...
    for i in range(0, k - 1):
        for j in range(0, i + 1):
            ax = fig_for_example.add_subplot(k - 1, k - 1, i * (k - 1) + j + 1)
    ...
    for i in range(0, k - 1):
        for j in range(0, i + 1):
            ax = fig_for_not_optimization.add_subplot(k - 1, k - 1, i * (k - 1) + j + 1)
            ax.xaxis.set_major_locator(MultipleLocator(1))
            ax.yaxis.set_major_locator(MultipleLocator(1))
            ax.set_xticklabels(x_ticker_labels, rotation=0)
            ax.set_yticklabels(y_ticker_labels, rotation=90)
    ...
    x_list = list(range(0, len(latin_best_phi_q_correlation_list)))
