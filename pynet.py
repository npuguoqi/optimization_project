...
"""
'''
you need to change the network name for different functions
for example:
        if you train the network for object, the code should be modified as
...
the aim of the design is to make it convenient to use the network in optimization algorithm, as we can use the network name to reload the network for prediction
'''
import numpy as np
import copy
import sys
...
if __name__ == '__main__':
    ...
    input_traindata = read_csv(r'xls_file\data_result_transpose.csv')
    output_traindata = read_csv(r'xls_file\fun_result_matrix.csv')
    input_testdata = read_csv(r'xls_file\data_test_transpose.csv')
    output_testdata = read_csv(r'xls_file\fun_test_matrix.csv')
    input_testdata_train = read_csv(r'xls_file\data_test_transpose_train.csv')
    output_testdata_train = read_csv(r'xls_file\fun_test_matrix_train.csv')
    ...
    netuse = NetUtilize(net, mu=1e-3, mu_dec=0.1, mu_inc=10, mu_max=1e10, iteration=2500, tol=1e-30, gradient=1e-10,gamma_criterion=1e-6, training_method='Bayesian-Regularization')
    ...
    if netuse.training_method == 'Bayesian-Regularization':
        ...
        file_object.close()
        #write gamma
        file_object = open(r'data_for_plot\for_regularization\gamma.txt', 'w')
